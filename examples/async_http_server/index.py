...
app = liberty.AsyncHttpServer()

@routes.get('/index')
async def index_handler(request):
	file = liberty.HtmlFile(path="html/index.html", encoding="utf-8")
	return app.html_response(text=file)


@routes.get('/index_change')
async def index_handler(request):
	file = liberty.HtmlFile(path="html/index_change.html", encoding="utf-8", name="Fried")
	return app.html_response(text=file)
